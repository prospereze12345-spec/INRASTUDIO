...
for (const p of pages) {
  ...
  // They might not have imported TikTok, Instagram, etc. at the top, since it was just removed by prior edits
  if (!content.includes('import { TikTok')) {
    content = content.replace(/(import { [A-Za-z0-9, \n]+) } from "lucide-react";/, '$1, TikTok, Instagram } from "lucide-react";');
  }
  ...
  const replacementFooterBlock = `         <div className="flex flex-wrap gap-12 sm:gap-24 uppercase text-xs tracking-widest font-mono shrink-0">
            ...
            <div className="flex flex-col gap-5">
              ...
              <a href="#" className="text-slate-300 hover:text-white transition-colors flex items-center gap-2 group"><Music2 className="w-4 h-4 text-slate-400" /> TikTok <ArrowRight className="w-3 h-3 -rotate-45 group-hover:text-cyan-400 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" /></a>
